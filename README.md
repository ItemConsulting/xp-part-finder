...
- __Take care to spell the new key correctly and with the full name__. The "Replace part" button is enabled when at
  ...
  width="900" style="border:1px solid #eaeaea">
...
- __Use the summary/undo page__: click the links and check each content to verify the operation (links open in new tab).
  ...
  screen/navigating away, the undo opportunity is lost - that includes using the undo function.

- Known bug: __the undo function on error items doesn't work perfectly__ (marked with a red cross icon ❌). On errors,
